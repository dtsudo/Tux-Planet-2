...
let Enemy_Level1: { getEnemy: (enemyId: number) => IEnemy } = {} as any;

((function () {
	let getEnemy = function (frameCount: number, enemyId: number): IEnemy {

		let thisEnemy: IEnemy;

		let processFrame = function (thisObj: IEnemy, enemyMapping: { [index: number]: IEnemy }, rngSeed: number, nextEnemyId: number): EnemyProcessFrameResult {
			...
			if (frameCount === 1000) {
				let enemy = Enemy_FlyamanitaBig.getEnemy(
					350 * 1024,
					nextEnemyId,
					rng);
				nextEnemyId++;
			...
			if (frameCount === 1500) {
				...
				enemy = Enemy_FlyamanitaBig.getEnemy(
					500 * 1024,
					nextEnemyId,
					rng);
				nextEnemyId++;
	...
	Enemy_Level1.getEnemy = function (enemyId: number): IEnemy {
