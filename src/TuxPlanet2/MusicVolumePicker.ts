...
let MusicVolumePickerUtil = {
	getMusicVolumePicker: function (xPos: number, yPos: number, initialVolume: number): MusicVolumePicker {
		...
		let render = function (displayOutput: IDisplayOutput) {
			if (currentVolume > 0)
				displayOutput.drawImage(GameImage.MusicOn, xPos, yPos);
			else
				displayOutput.drawImage(GameImage.MusicOff, xPos, yPos);
