...
let Enemy_OgJumpy = ((function () {
	...
	let getEnemy = function (
		...
		let arcTangentScaled = function (x: number, y: number): number {
			if (x === 0 && y === 0)
				return 0;
		...
		let processFrame: enemyProcessFrameFunction = function ({ thisObj, enemyMapping, rngSeed, nextEnemyId, difficulty, playerState, tilemap, soundOutput }) {
			...
			if (shouldAttack && screenWipeCountdown === null) {
				...
				switch (difficulty) {
					case Difficulty.Easy: angleArray = [-1, 1]; break;
					case Difficulty.Normal: angleArray = [-1, 0, 1]; break;
					case Difficulty.Hard: angleArray = [-2, -1, 0, 1, 2]; break;
				}
	...
	return {
		getEnemy: function ({ xInitialMibi, yInitialMibi, difficulty, enemyId }: { xInitialMibi: number, yInitialMibi: number, difficulty: Difficulty, enemyId: number }): IEnemy {
