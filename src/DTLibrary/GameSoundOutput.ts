...
let GameSoundOutput = {
	getSoundOutput: function (): ISoundOutput & ISoundProcessing {
		"use strict";

		let soundDictionary: { [index: number]: HTMLAudioElement[] } = {};

		let numberOfAudioObjectsLoaded = 0;

		let hasFinishedLoading = false;

		let loadSounds = function (): boolean {
			let soundNamesArray: GameSound[] = GameSoundUtil.getSounds();
		...
		let playSound = function (sound: GameSound, volume: number) {
			...
			let soundArray: HTMLAudioElement[] = soundDictionary[sound];
		...
		let setSoundVolume = function (volume: number) {
			if (volume < 0)
		...
		let processFrame = function () {
			currentSoundVolume = VolumeUtil.getVolumeSmoothed(
				currentSoundVolume,
				desiredSoundVolume);
		};
