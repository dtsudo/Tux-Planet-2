...
let GameKeyboard = {
	getKeyboard: function (disableArrowKeyScrolling: boolean): IKeyboard {
		"use strict";

		let keysBeingPressed: Key[] = [];

		let mapKeyToCanonicalKey = function (key: string) {
			if (key === "A")
				return "a";
			if (key === "B")
				return "b";
		...
		let keyUpHandler = function (e: KeyboardEvent) {
			let key: Key = KeyMapping[mapKeyToCanonicalKey(e.key)];
		...
		let isPressed = function (key: Key): boolean {
			for (let i = 0; i < keysBeingPressed.length; i++) {
				if (keysBeingPressed[i] === key)
					return true;
			}
