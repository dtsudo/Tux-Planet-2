...
let GameMusicOutput = {
	getMusicOutput: function (): IMusicOutput & IMusicProcessing {
		"use strict";

		let musicDictionary: { [index: number]: HTMLAudioElement } = {};

		let numberOfAudioObjectsLoaded = 0;

		let loadMusic = function () {
			let musicNamesArray: GameMusic[] = GameMusicUtil.getMusic();
		...
		*/
		let currentFadeInAndOutVolume: number = 0.0;
		...
		/*
			...
			the game is paused)
		*/
		let currentMusicVolume: number = 0;
		...
		*/
		let intendedMusicVolume: number = 0;

		// From 0 to 100
		let globalMusicVolume: number = 0;

		let playMusic = function (music: GameMusic, volume: number) {
			intendedMusic = music;
		...
		let setMusicVolume = function (volume: number) {
