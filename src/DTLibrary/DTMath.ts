...
let DTMath = ((function () {
	...
	let normalizeDegreesScaled = function (degreesScaled: number): number {
	...
	let sineScaled = function (degreesScaled: number): number {
	...
	let cosineScaled = function (degreesScaled: number): number {
		if (degreesScaled < 0)
	...
	let arcTangentScaled = function (x: number, y: number): number {
		...
		if (x > 0 && y > 0)
		{
			...
			if (x >= 15 * 1000 * 1000 || y >= 15 * 1000 * 1000)
			{
				...
				let resultLong = Math.floor((yLong * 128) / xLong);
				if (resultLong >= 1000 * 1000 * 1000)
					resultLong = 1000 * 1000 * 1000;
				result = resultLong;
			}
			else
